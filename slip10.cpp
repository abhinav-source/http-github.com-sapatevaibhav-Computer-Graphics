// Implement following 2D transformations on the object with respect to axis a.Scaling b.Rotation about arbitrary point c.Reflection.
...
void rotation()
{
    ...
    {
        float newX = ((xpoint[i] - cx) * cos(the) - ((ypoint[i] - cy) * sin(the))) + cx;
        float newY = ((xpoint[i] - cx) * sin(the) + ((ypoint[i] - cy) * cos(the))) + cy;
